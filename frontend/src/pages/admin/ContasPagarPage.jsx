...
import { 
  Plus, Search, CreditCard, Calendar, CheckCircle2, AlertCircle, Clock, Edit, Trash2, X, Filter, Paperclip, UserPlus, Landmark, History, DollarSign, CircleDot, FileDown, TrendingUp, TrendingDown, FileText, Receipt, Loader2
} from "lucide-react";
    ...
    try {
      ...
      if (querParcelar) {
        ...
        await anexosRef.current?.flushPending(editingConta.id);
          ...
          const _resp = await axios.post(`${API}/admin/contas-pagar`, dataToSend);
  ...
  const exportarConta = async (id, tipo) => {
    ...
    try {
      const endpoint = tipo === "recibo"
        ? `${API}/export/recibo/contas_pagar/${id}`
        : `${API}/export/individual/contas_pagar/${id}`;
      const response = await axios.get(endpoint, { responseType: "blob" });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      const cd = response.headers["content-disposition"];
      let filename = `CRA_conta_pagar_${tipo}_${id.slice(0, 8)}.pdf`;
      if (cd) {
